        ...
        <template #container="{ closeCallback }">
            <div class="flex flex-col px-8 py-8 gap-6 rounded-2xl"
                ...
                <div class="inline-flex flex-col gap-2">
                    ...
                    <InputText id="username" v-model="account.username"
                        class="!bg-white/20 !border-0 !p-4 !text-primary-50 w-80"></InputText>
                </div>
                <div class="inline-flex flex-col gap-2">
                    <label for="password" class="text-primary-50 font-semibold">Password</label>
                    <InputText id="password" v-model="account.password"
                        class="!bg-white/20 !border-0 !p-4 !text-primary-50 w-80" type="password">
                    </InputText>
                </div>
                <div class="flex items-center gap-4">
                    ...
                    <Button label="Sign-In" @click="(event) => sendForm(closeCallback)" text
                        class="!p-4 w-full !text-primary-50 !border !border-white/30 hover:!bg-white/10"></Button>
                </div>
            </div>
        </template>
...
const account = ref({ ...initialAccountState });

function resetForm() {
  account.value = { ...initialAccountState };
}

function sendForm(closeCallback) {
  console.log("Form submitted:", account.value);
  toast.add({
    severity: 'success',
    summary: 'Info',
